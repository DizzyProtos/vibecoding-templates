# Safety Rules for Team Mood & Check-In API
...
- **Do not** change domain rules, validation rules, or entity semantics in a way that would break existing callers or stored data (e.g. mood scale, check-in fields, summary aggregation).
- **Do not** delete or disable existing tests; add or update tests when behavior changes.
- **Do not** refactor in a way that changes observable behavior (e.g. changing status codes, error shapes, or response fields) without explicit user approval.
