# Safety Rules for Wasted Time Tracker API
...
- **Do not** change semantics of `WasteSession` or `AlternativeUse` entities in a way that breaks existing clients or stored data.
...
- **Add tests** for new behavior; keep or update tests for modified behavior, especially around duration calculations and session lifecycle rules.
